...
foreach ($blocks as $block) {
    ...
    $block_texture[$block] = $config->get("block.{$block}.block-texture");
    $block_texture[$block] .= $config->get("block.{$block}.block-padding");

    $block_container[$block] = $config->get("block.{$block}.block-container");
}
...
$body_classes .= $this['config']->get('sticky_navbar') ? ' tm-navbar-fixed' : '';
$body_classes .= $this['config']->get('navbar_bg') ? ' tm-navbar-bg' : '';
...
$body_config = array();
$body_config['stickynav'] = (int) $config->get('sticky_navbar', 0);
$body_config['centereddropdown'] = (int) $config->get('centered_dropdown', 0);
...
$this['asset']->addFile('js', 'js:menuAim.js');
